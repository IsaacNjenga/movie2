function Header()
{
  ...
  return(
<div>
<a href = "3D.html">3D Movies <i class = "material-icons">arrow_forward</i></a>
</div>    
  )
}
...
function App() {
    ...
    React.useEffect(() => {
      const fetchBackgroundImage = async () => {
        try {
          const response = await fetch(`https://yts.mx/api/v2/list_movies.json?page=${counter}`);
    ...
    React.useEffect(() => {
      const fetchData = async () => {
        try {
          const movieResponse = await fetch(`https://yts.mx/api/v2/list_movies.json?page=${count}`);
    ...
    function url()
    {
      window.open(suggestion[0].url, '_blank')
    }
